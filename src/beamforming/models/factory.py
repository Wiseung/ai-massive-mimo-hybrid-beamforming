...
def build_model(model_cfg: dict, data_cfg: dict) -> torch.nn.Module:
    ...
    if name == "sionna_ofdm_residual_rzf":
        return SionnaOFDMResidualRZFBeamformer(
            num_users=common["num_users"],
            num_bs_ant=common["num_bs_ant"],
            hidden_dim=int(model_cfg.get("hidden_dim", 128)),
    ...
    if name == "sionna_ofdm_unfolded_lite":
        return SionnaOFDMUnfoldedLiteBeamformer(
            ...
            init_method=str(model_cfg.get("init_method", "wmmse_iter_2")),
            learnable_step_size=bool(model_cfg.get("learnable_step_size", True)),
            step_size_init=float(model_cfg.get("step_size_init", 0.05)),
            condition_on_snr=bool(model_cfg.get("condition_on_snr", True)),
        )
