#!/usr/bin/env python
"""Compare baseline, beamforming, and receiver-chain Sionna-native artifacts."""
...
def main() -> None:
    ...
    if receiver is not None:
        comparison_rows.append(
            {
                "artifact": "beamforming_receiver_chain_v2",
    ...
    suffix = "_v2" if receiver is not None else ""
    comparison_csv = out_dir / f"native_chain_comparison{suffix}.csv"
    ...
    ok_metrics = metrics[metrics["method"].isin(["project_rzf", "project_wmmse_iter_5", "no_precoding", "project_wmmse_iter_2"])].copy()
    plt.figure(figsize=(7, 4))
    plt.bar(ok_metrics["method"], ok_metrics["symbol_mse"])
    plt.ylabel("symbol_mse")
    plt.tight_layout()
    plt.savefig(out_dir / f"mse_by_method{suffix}.png", dpi=160)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(out_dir / f"sinr_by_method{suffix}.png", dpi=160)
    plt.close()

    status_table = out_dir / "receiver_chain_status_table.csv"
    status_cols = [col for col in ["method", "native_receiver_success", "used_sionna_channel", "used_sionna_estimator", "used_sionna_equalizer", "used_sionna_demapper", "fallback_used", "fallback_stage", "fallback_reason"] if col in metrics.columns]
    metrics[status_cols].to_csv(status_table, index=False)

    best_method = ok_metrics.sort_values("approximate_sum_rate", ascending=False).iloc[0]
    native_no_prec_ok = bool(receiver and any(row["method"] == "no_precoding" and row["native_receiver_success"] for row in receiver["metrics"]))
    native_beam_ok = bool(receiver and any(row["method"] != "no_precoding" and row["native_receiver_success"] for row in receiver["metrics"]))
    lines = [
        "# Sionna Native Chain Comparison",
        "",
        f"1. receiver chain succeeds under no_precoding: `{native_no_prec_ok}`",
        f"2. receiver chain succeeds under beamforming methods: `{native_beam_ok}`",
        f"3. beamforming chain truly uses Sionna ResourceGrid: `{beamforming['used_sionna_resource_grid']}`",
        f"4. beamforming chain truly uses Sionna OFDMChannel/ApplyOFDMChannel path: `{bool(receiver and receiver['used_sionna_channel'])}`",
        ...
        f"8. current learned-beamformer insertion recommendation: `{True}`",
        f"9. next step should connect residual_rzf learned model: `{True}`",
        "",
        "## Receiver Status",
        f"- native receiver success methods: `{receiver['methods_successful_under_native_receiver'] if receiver else []}`",
        f"- fallback-only methods: `{receiver['methods_fallback_only'] if receiver else []}`",
        f"- first native failure stage: `{receiver['native_failure_stage'] if receiver else ''}`",
        f"- first native failure reason: `{receiver['native_failure_reason'] if receiver else ''}`",
        "",
        "## Fallback Notes",
    ]
    for note in (receiver["notes"] if receiver is not None else beamforming["notes"]):
        lines.append(f"- {note}")
    (out_dir / f"native_chain_comparison{suffix}.md").write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")
